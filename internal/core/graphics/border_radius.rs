...
/// Larger `|k|` is clamped to this.
/// `2^16` is already indistinguishable from `Square` (or from `Notch`) at any radius,
/// and an unclamped `n` overflows to infinity.
const MAX_SUPERELLIPSE_K: f32 = 16.0;

/// The exponent of the superellipse curve for the CSS `corner-shape: superellipse(k)`
/// parameter `k`: `n = 2^|k|`, so `n >= 1` always.
///
/// The sign of `k` selects the convex or the concave branch of the family, rather than
/// changing the exponent. See [`corner_boundary`].
...
/// Shared with [`corner_path`](super::corner_path)'s `superellipse_at`, which explains why
/// `s` is used instead of the CSS spec's `t`.
pub(crate) fn superellipse_pq(n: f32, s: f32) -> (f32, f32) {
    // Factoring out the larger of the two keeps the power in `0..=1`, where a large `n`
    // can't overflow it.
...
///
/// A positive `k` gives the convex branch, `(1 - x/r)^n + (1 - y/r)^n = 1`, which bows out
/// toward the corner tip. A negative `k` mirrors that about the bevel chord to the concave
/// branch, `(x/r)^n + (y/r)^n = 1`.
/// So `superellipse(-k)` is the exact opposite of `superellipse(k)`, which is what makes
/// `superellipse(-1)` the circular `Scoop` facing `superellipse(1)`'s circular `Round`.
...
///
/// Every [`CornerShape`](crate::items::CornerShape) is one value of the CSS
/// `corner-shape: superellipse(k)` parameter: `Notch` is `k = -∞`, `Scoop` is `k = -1`,
/// `Bevel` is `k = 0`, `Round` is `k = 1`, `Squircle` is `k = 2`, and `Square` is `k = ∞`.
/// The named shapes keep the closed form they happen to have, which is both cheaper and
/// exact. Only `Squircle` and `Superellipse` evaluate the superellipse itself.
...
/// Position (`x/r`, `y/r`) and unit tangent of the superellipse family, in the corner's
/// frame, at ray parameter `s`.
/// Scalar counterpart of [`corner_path`](super::corner_path)'s `superellipse_at`, which
/// returns a `Vector` for its path-building callers. This one only needs the two
/// components.
...
/// distance `wn` into the material (border width `/ r`). Evaluated at normalized row `yn`
/// (both `/ r`).
/// `None` when `yn` is closer to the tip than the offset curve ever reaches. See
/// [`inner_corner_boundary`].
///
/// Solved by bisection on the ray parameter `s`. The offset point's `y` is monotonic in
/// `s` for `wn` well under 1. That covers every border width that isn't wider than the
/// radius it's inset from.
fn superellipse_offset_x(k: f32, n: f32, wn: f32, yn: f32) -> Option<f32> {
    let offset_y = |s: f32| -> f32 {
        ...
        // The inward normal: the tangent rotated -90 degrees, which points from the curve
        // into the material on both the convex and the concave branch.
        pos.1 - wn * tangent.0
    };
    if yn < offset_y(0.0) {
        return None;
...
///
/// A shape whose curve meets the straight edge tangentially (`Round`, `Squircle`, convex
/// `Superellipse`) reaches it exactly at `y == r`, as does `Square`, which has no curve to
/// offset. The rest meet the edge at an angle, so their true perpendicular offset doesn't
/// come back down to `r - r2` until past the corner's own radius.
/// [`corner_boundary`] already widens the zone it evaluates `Notch` over by this amount, and
/// the software renderer's `draw_rounded_rectangle_line` does the same for every shape
...
/// perpendicular to itself by the border width `r - r2`, the way a stroked path is offset.
/// That's what keeps a border a uniform width all the way around a corner, whatever its
/// shape.
///
/// A shape that meets the straight edge at an angle rather than tangentially has an offset
/// that only reaches the edge's own flat width past `y == r`; see [`inner_corner_extra`] for
/// how far past. Evaluating this function beyond `y == r` for those shapes, rather than
/// clamping to it, is what the software renderer's `draw_rounded_rectangle_line` relies on
/// to avoid a seam where the corner meets the straight run of the border.
pub fn inner_corner_boundary(shape: CornerShape, r: f32, r2: f32, y: f32) -> f32 {
    ...
    match shape {
        CornerShape::Square => {
            // A plain mitered corner: offsetting the two edges by `w` meets at `(w, w)`.
            // No fill shows through above that miter, the same as the `r2 <= 0` case
            // above; this function only ever sees `y < w` in that range.
            if y < w { r } else { w }
        }
        CornerShape::Notch => {
            // Same miter threshold as `Square` for `y < w`. Above it, offsetting the
            // reflex corner at `(r, r)` outward by `w` moves this wall to `r + w`.
            if y < w { r } else { r + w }
        }
        CornerShape::Bevel => {
        ...
        CornerShape::Round => {
            // The circle centered on `(r, r)` shrunk from radius `r` to `r - w`.
            // That circle doesn't reach rows closer than `w` to the tip.
...
mod tests {
    ...
    /// The closed-form perpendicular offset by `w` for the three shapes it's easy to derive
    /// by hand, each checked independently of `inner_corner_boundary`'s own derivation.
    /// Sampled up to each shape's own `y == r` plus [`inner_corner_extra`], since that's the
    /// range `inner_corner_boundary` now evaluates them over.
    #[test]
    fn inner_corner_boundary_matches_closed_form() {
        ...
        let cases: &[(CornerShape, fn(f32, f32, f32) -> f32)] = &[
            ...
            // A circular arc centered on the tip, radius grown from `r` to `r + w`.
            (CornerShape::Scoop, |r, w, y| {
                if y < w { r } else { ((r + w) * (r + w) - y * y).max(0.0).sqrt() }
            }),
            // A circular arc centered on `(r, r)`, radius shrunk from `r` to `r - w`.
            // That circle doesn't reach rows closer than `w` to the tip.
    ...
    /// `Notch` and `Square` are exact miters, so their offset is a two-piece step in both
    /// cases. Nothing shows through below `y = w`, where the row is nearer the straight
    /// edge than either wall's own offset. Above it the offset is constant, independent
    /// of the bisection the smooth shapes use.
    #[test]
    ...
    /// `y == r + inner_corner_extra(...)` is where the scanline renderer hands off from
    /// `inner_corner_boundary` to the border's flat, non-corner width `w` (that extra is
    /// zero for shapes that reach `w` at `y == r` itself). Every shape but `Notch` has to
    /// land on `w` exactly there, or the fill boundary steps where the corner meets the
    /// straight run.
    ///
    /// A concave `Superellipse` (negative `k`) has this same gap: `superellipse_offset_x`
    /// solves for a true perpendicular offset that doesn't reach `w` at `y == r` either, and
    /// `inner_corner_extra` doesn't (yet) widen the zone for it. Not covered here.
    #[test]
    fn inner_corner_boundary_meets_flat_border_at_r() {
        ...
        for shape in shapes {
            for r in [10.0f32, 40.0, 128.0] {
                // `superellipse_offset_x`'s bisection assumes the offset is monotonic in its
                // ray parameter, which the module doc already qualifies as holding only for
                // `wn` "well under 1"; `r * 0.6` is wide enough to break that assumption for
                // some `n`, independent of the gap this test pins.
    ...
    /// curve directly, by nearest-point-on-polyline, not by reusing
    /// `inner_corner_boundary`'s own offset math.
    /// That catches the same "scaled about the wrong point" mistake the closed-form tests
    /// above can't reach for `Squircle` and `Superellipse`, which have no simple closed
    /// form to check against.
    #[test]
    fn inner_corner_boundary_is_a_uniform_perpendicular_offset() {
        use crate::graphics::border_radius::CornerShape;
        // `Notch` and `Square` step at `y == w`. Linearly bridging that jump, the way
        // sampling every other (continuous) shape into a polyline does, would put spurious
        // close points next to the step.
        // `Bevel` and `Scoop` don't meet the straight edge tangentially, so their true
        // offset runs past `y == r` before meeting the next wall, the same way `Notch`'s
        // does; the polyline below is sampled only up to `r`, so it can't find that part of
        // the curve either.
