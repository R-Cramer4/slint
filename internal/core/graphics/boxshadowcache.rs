...
impl Ord for BoxShadowOptions {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        // `CornerShape` isn't a fieldless enum (its Rust-only `Superellipse` variant carries
        // an `f32`), so it can't be cast `as u32`; order by variant index and, for
        // `Superellipse`, its exponent bits.
        fn corner_shape_key(shape: crate::items::CornerShape) -> (u32, u32) {
