...
pub(super) fn draw_rounded_rectangle_line(
    ...
    // around, whatever the shape.
    // `Round` keeps its exact fixed-point formula: the general one below reduces to it
    // algebraically, but this avoids the sqrt-of-negative dance and floating-point drift
    // for the common case.
    let calculate_xxxx = |r: i16, shape: i_slint_core::items::CornerShape, y: i16| {
        if matches!(shape, i_slint_core::items::CornerShape::Round) {
            ...
            // The far sample normally lands one row below the near one, `y + 1`. Both
            // `corner_boundary` and `inner_corner_boundary` are right-continuous, so that's
            // exactly the row's far edge, for every shape except `Notch`.
            // `Notch`'s boundary steps down, at `y == r` outer and `y == border_f` inner,
            // rather than curving down, and sampling exactly on that step reads the value
            // just past it. That smears a full-width gradient across the row the step falls
            // in, instead of leaving it flat like the rows above it.
            // Stepping the far sample back onto the near sample's side of a step it landed
            // on avoids that. Every other shape has no step to land on.
    ...
    // A row is part of a corner's curve within `radius` of the edge, same as ever. A shape
    // that doesn't taper into the straight edge (`Notch`, `Bevel`, `Scoop`) has a border
    // whose inner wall runs *across* the corner, some distance past the radius, rather than
    // tapering away before it; see `inner_corner_extra`. Widening the zone by that amount
    // reaches `calculate_xxxx` for those rows too.
    let in_corner_zone = |radius: i16, shape: i_slint_core::items::CornerShape, y: PhysicalLength| {
