...
pub fn rounded_rect_path(
    ...
    to_skia_path(&i_slint_core::graphics::corner_path::rounded_rect_path(
        rect.origin.x,
        rect.origin.y,
        rect.size.width,
        rect.size.height,
...
/// Builds the exact path for `rect` (already grown by `spread`) around the ungrown
/// `radius`/`corner_shape` — the shape a box shadow's spread grows into. See
/// [`i_slint_core::graphics::corner_path::spread_rounded_rect_path`].
pub fn spread_rounded_rect_path(
    rect: &PhysicalRect,
    radius: &PhysicalBorderRadius,
    corner_shape: i_slint_core::lengths::CornerShapes,
    spread: f32,
) -> skia_safe::Path {
    to_skia_path(&i_slint_core::graphics::corner_path::spread_rounded_rect_path(
        rect.origin.x,
        rect.origin.y,
        rect.size.width,
        rect.size.height,
...
fn to_skia_path(path: &lyon_path::Path) -> skia_safe::Path {
    let mut builder = skia_safe::PathBuilder::new();
