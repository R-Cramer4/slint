...
/// Fills in `per_corner_properties` with a binding to `uniform_property`, so it acts as their default.
///
/// Only applies when `uniform_property` is set on a `Rectangle` alongside at least one of
/// `trigger_properties`. Setting either one selects the native item that reads all eight per-corner properties.
