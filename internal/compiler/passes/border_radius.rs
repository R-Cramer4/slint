...
/// Fills in `per_corner_properties` (e.g. the four `border-*-radius` ones) with a binding
/// to `uniform_property` (e.g. `border-radius`), so it acts as their default.
///
/// Only applies when `uniform_property` is set on a `Rectangle` alongside at least one of
/// `trigger_properties`, the radius or corner-shape per-corner properties. Setting either
/// one selects the native item that reads all eight per-corner properties.
fn fan_out_to_corners(
    elem: &ElementRc,
    uniform_property: &str,
    per_corner_properties: [&str; 4],
    ...
    if bty.name == "Rectangle"
        ...
        && trigger_properties.iter().any(|property_name| {
...
pub fn handle_border_radius(
    ...
    let trigger_properties: Vec<&str> =
        BORDER_RADIUS_PROPERTIES.iter().chain(CORNER_SHAPE_PROPERTIES.iter()).copied().collect();
