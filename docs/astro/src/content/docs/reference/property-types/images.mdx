...
GIF, animated PNG and animated WebP files play back automatically when assigned to an
`Image` element's `source`; see the <Link type="Image" label="Image element"/> for the
`running` property that pauses and resumes playback. This requires the `animated-images`
feature (see below) and isn't supported on the web, where animated sources show only
their first frame. For sprite sheets with a uniform frame size and delay, the
[sprite-sheet example](https://github.com/slint-ui/slint/tree/master/examples/sprite-sheet)
remains the right tool, and works everywhere including the web.

Animated formats are decoded eagerly: every frame is kept in memory as a separate
RGBA buffer for as long as the image is in use, so a large or long animation can use
significantly more memory than its file size suggests (a 480×270 GIF with 60 frames is
already around 31 MB decoded, for example). Compile-time texture embedding for the
software renderer (`@image-url` on MCU/`no_std` targets) only ever keeps the first
frame, since those targets typically have no allocator for a frame array; the compiler
...
Animated GIF, animated PNG and animated WebP playback is a separate, opt-in
`animated-images` Cargo feature (independent of `image-default-formats`), since it
pulls in additional decoder code and the per-item playback machinery. It has no
effect on the web.
:::
</TabItem>
